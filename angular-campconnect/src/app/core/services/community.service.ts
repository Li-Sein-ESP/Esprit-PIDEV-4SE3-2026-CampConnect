...
export class CommunityService {
    ...
    /** Création de post + modération IA côté serveur peut dépasser 30 s (images / Python). */
    private readonly postWriteTimeoutMs = 120000;
    private readonly postWriteContext = new HttpContext().set(extendedHttpTimeoutMs, this.postWriteTimeoutMs);
    ...
    createPost(post: Omit<Post, 'id' | 'createdAt' | 'likes' | 'replies' | 'views'>): Observable<Post> {
        ...
        return this.http.post<any>(this.apiUrl, payload, { context: this.postWriteContext }).pipe(
            map(p => this.mapBackendToFrontendPost(p))
        );
    }
    ...
    uploadPostImage(file: File): Observable<{ url: string }> {
        ...
        return this.http.post<{ url: string }>(`${environment.apiUrl}/upload`, formData, {
            context: this.postWriteContext
        });
    }

    createReply(
        reply: Omit<Reply, 'id' | 'createdAt' | 'likes'> & { authorUsername?: string }
    ): Observable<Reply> {
        const payload = {
            postId: reply.postId,
