...
export class GearCreateComponent implements OnInit, OnDestroy {
    ...
    onSubmit(): void {
        ...
        if (this.selectedFile) {
            ...
            this.gearService.uploadImage(this.selectedFile).subscribe({
                next: (res) => {
                    this.isUploadingImage = false;
                    this.doSubmit([res.url]);
                },
                error: (err) => {
    ...
    private doSubmit(imageUrls: string[]): void {
        ...
        this.gearService.createGear(request).subscribe({
            ...
            error: (err) => {
                this.isSubmitting = false;
                // Only keep field-level validation errors, global errors handled by interceptor
