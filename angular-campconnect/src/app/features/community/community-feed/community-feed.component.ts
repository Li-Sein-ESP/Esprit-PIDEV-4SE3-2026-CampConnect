...
export class CommunityFeedComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.authService.isLoggedIn()) {
            this.authService.refreshProfileFromServer().subscribe(() => {
                this.currentUser = this.authService.currentUserValue;
        ...
        this.ngZone.run(() => {
            ...
            fetchObservable.subscribe({
                ...
                error: (err) => {
                    ...
                    if (this.feedMode === 'FOLLOWING' && err?.status === 401) {
                        alert('Session expirée ou non connecté — reconnectez-vous pour « Mon feed ».');
                    }
                    this.loading = false;
                    this.cdr.detectChanges();
                }
    ...
    setFeedMode(mode: 'ALL' | 'FOLLOWING'): void {
        if (mode === 'FOLLOWING' && !this.currentUser?.id) {
            alert('Connectez-vous pour voir le fil des comptes que vous suivez.');
            return;
        }
