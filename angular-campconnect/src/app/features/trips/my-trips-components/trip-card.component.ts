...
@Component({
  ...
  template: `
    <article
      class="flex flex-col rounded-3xl border border-white bg-white/70 backdrop-blur-md shadow-sm transition-all hover:shadow-lg hover:border-emerald-100 md:flex-row group overflow-hidden"
      *ngIf="trip"
    >
      <!-- Image Section -->
      <div
        class="relative w-full overflow-hidden md:w-72 cursor-pointer"
        [routerLink]="['/trips', trip.id]"
      >
        <img
          [src]="trip.imageUrl"
          [alt]="trip.title"
          class="h-56 w-full object-cover transition-transform duration-700 group-hover:scale-110 md:h-full"
        />
        <div
      ...
      <div class="flex flex-1 flex-col justify-between p-8">
        <div>
          ...
          >
            ...
            >
              <h2
                class="text-2xl font-extrabold text-emerald-950 leading-tight group-hover:text-emerald-800 transition-colors"
              >
                {{ trip.title }}
              </h2>
              <div
                class="flex items-center gap-1.5 text-sm font-semibold text-slate-500"
              >
                <lucide-icon
                  [img]="MapPinIcon"
                  size="14"
          ...
          <div class="mt-8 grid gap-6 grid-cols-2 md:grid-cols-3">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <div class="text-xs font-bold" [ngClass]="difficultyClasses">
                  {{ trip.difficulty }}
                </div>
        ...
        <!-- Footer Actions -->
        <div
          class="mt-8 flex flex-col items-start justify-between gap-4 border-t border-slate-100 pt-6 md:flex-row md:items-center"
        >
          <div
          ...
          >
            ...
            ></div>
            <span
              class="text-[11px] font-bold text-emerald-800 tracking-wide"
              >{{ trip.timelineLabel }}</span
            >
          </div>
          ...
          <div class="flex items-center gap-3 w-full md:w-auto">
            ...
            <!-- Additional quick actions: My Trips and Nearby -->
            <button
              [routerLink]="['/trips']"
              class="hidden md:inline-flex items-center gap-2 ml-2 rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all"
            >
              My Trips
            </button>

            <button
              [routerLink]="['/plan-trip', trip.id, 'nearby']"
              [queryParams]="{ date: trip.startDate }"
              class="hidden md:inline-flex items-center gap-2 ml-2 rounded-md bg-emerald-50 border border-emerald-100 px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-100 transition-all"
            >
              Nearby on {{ trip.startDate }}
            </button>
          </div>
        </div>
      </div>
